...
const getUserName = (tweet: HTMLElement) => {
    return (
        tweet.querySelector(
            '[data-testid="User-Name"]>div:nth-child(1) a[role="link"] span',
        )?.textContent || undefined
    );
};
...
export const getTweetInfos = (tweet: HTMLElement) => {
    ...
    const tweetDom = tweet.querySelector(
...
export const getStatusPageInfos = () => {
    ...
    const focussedTweet = document.querySelector(
    ...
    const focussedTweetUserId = focussedTweet
        ? getUserId(focussedTweet as HTMLElement)
        : null;
    return {
        isStatusPage,
        focussedTweet,
