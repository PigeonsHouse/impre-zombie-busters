...
function App() {
  ...
  useEffect(() => {
    (async () => {
      setData(
        decodeUserDataList(await getFromStorage(StorageKey.INVISIBLE_USERS)),
      );
  ...
  const ngWordTweetChangeHandler = useCallback(
  ...
  const ngWordUsernameChangeHandler = useCallback(
    (e: ChangeEvent<HTMLTextAreaElement>) => {
  ...
  return (
    <AppContainer>
      <NgWordEditor
        title="本文のNGワード"
        ngWords={ngWordTweet}
        onChangeNgWords={ngWordTweetChangeHandler}
        saveNgWords={saveNgWordTweetHandler}
      />
      <NgWordEditor
        title="ユーザ名のNGワード"
        ngWords={ngWordUsername}
        onChangeNgWords={ngWordUsernameChangeHandler}
        saveNgWords={saveNgWordUsernameHandler}
      />
      <UserList
